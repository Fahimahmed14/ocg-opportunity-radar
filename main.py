...
def build_report(results, profile):
    """
    ...
    if not top_results:
        ...
        message += (
            "No strong opportunities found today.\n\n"
            "The radar searched multiple opportunity "
            "categories and filtered the results using AI.\n\n"
            "Try again tomorrow."
        )
...
def main():
    ...
    if not opportunities:
        ...
        message = (
            "🌊 OCG OPPORTUNITY RADAR\n\n"
            "⚠️ No candidates were collected today.\n\n"
            "The search collector may have encountered "
            "a temporary search-engine problem."
        )
    ...
    # --------------------------------------------------
    # Limit candidates before AI
    # --------------------------------------------------

    # Keep the first 40 candidates.
    # The collector already performed keyword filtering.
    ai_candidates = opportunities[:40]

    print(
        f"Sending {len(ai_candidates)} candidates "
        "to OpenRouter AI..."
    )

    # --------------------------------------------------
    # AI ranking in batches of 20
    # --------------------------------------------------
    ...
    for start in range(
        ...
        try:
            ...
            rankings = rank_opportunities(
                batch,
                profile
            )

            # Convert batch indexes into global indexes
            ...
            print(
                f"AI batch failed: {error}"
            )

    print(
        f"\nAI returned "
        f"{len(all_rankings)} rankings."
    )
